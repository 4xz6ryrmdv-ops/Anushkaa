          ...
          {POLAROID_IMAGES.map((img, i) => (
            ...
            >
              ...
              <div className="text-center">
                <p className="font-serif italic text-gray-600 text-lg sm:text-xl mb-2">A Beautiful Memory</p>
  ...
  return (
    <div className="relative group/sig h-32 w-32 flex items-center justify-center">
      <svg width="120" height="120" viewBox="0 0 100 100" className="text-gold fill-none overflow-visible">
        ...
        {/* Artistic Cursive A */}
        <motion.path
          d="M20 70 C 25 30, 45 15, 55 20 C 65 25, 75 50, 70 80 M45 45 C 55 42, 65 42, 75 48"
        ...
        {/* Particle Trail Following the Pen */}
        {[...Array(5)].map((_, i) => (
          <motion.circle
            key={`sparkle-a-${i}`}
            r="1"
          ...
          >
            <animateMotion
              dur="3s"
              path="M20 70 C 25 30, 45 15, 55 20 C 65 25, 75 50, 70 80 M45 45 C 55 42, 65 42, 75 48"
              rotate="auto"
              repeatCount="1"
              begin="0s"
            />
      ...
      <motion.div 
        ...
        className="absolute inset-0 bg-gradient-to-tr from-gold/20 via-transparent to-white/10 blur-2xl rounded-full -z-10"
      />
    </div>
  );
};
...
const SignatureH = () => (
  <div className="relative group/sig h-32 w-32 flex items-center justify-center">
    <svg width="120" height="120" viewBox="0 0 100 100" className="text-gold fill-none overflow-visible font-serif">
      <motion.path
        d="M25 25 C 28 40, 28 60, 25 85 M75 25 C 72 40, 72 60, 75 85 M25 55 C 40 50, 60 50, 75 55"
        stroke="url(#gold-grad)"
        strokeWidth="2"
        strokeLinecap="round"
        filter="url(#beauty-glow)"
      ...
      {[...Array(5)].map((_, i) => (
        ...
        >
          <animateMotion
            dur="3s"
            path="M25 25 C 28 40, 28 60, 25 85 M75 25 C 72 40, 72 60, 75 85 M25 55 C 40 50, 60 50, 75 55"
            rotate="auto"
            repeatCount="1"
            begin="0.5s"
          />
    ...
    <motion.div 
      ...
      transition={{ duration: 10, repeat: Infinity, ease: "linear" }}
      className="absolute inset-0 bg-gradient-to-bl from-white/10 via-transparent to-gold/20 blur-2xl rounded-full -z-10"
    />
  </div>
);
...
const BloomingFlower = ({ className, delay = 0 }: { className: string, delay?: number }) => (
  <motion.div
    ...
    className={className}
  >
    <Flower2 size={24} className="text-gold/40" />
  </motion.div>
);
...
const TheFutureDeal = () => {
  const dealPoints = useMemo(() => [
    {
      ...
      description: "A promise whispered in the gym, now etched in the stars: If the world hasn't found us by thirty, we find each other. Not out of chance, but because it was always meant to be.",
      icon: <Clock size={32} className="text-gold" />,
    },
    {
      title: "Our Sanctuary",
      description: "A home in the States where the garden always blooms. A haven filled with the pitter-patter of one dog, one cat, and the echoes of our shared laughter.",
      icon: <Home size={32} className="text-gold" />,
    },
    {
      title: "The Saturday Vow",
      description: "Every Saturday night, the world stops for us. A fresh bouquet of her favorites, a candlelit date, and the simple magic of being present. Your happiness is my only mission.",
      icon: <Flower2 size={32} className="text-gold" />,
    },
    {
      title: "The Sunday Rhythm",
      description: "Sundays belong to the kitchen. Slow mornings, shared recipes, and the quiet intimacy of building a life, one meal at a time. A soul-level connection in the simplest moments.",
  ...
  const butterflies = useMemo(() => [...Array(30)].map((_, i) => ({
    ...
    xStart: 50 + (Math.random() * 40 - 20),
    xEnd: 50 + (Math.random() * 100 - 50),
    yEnd: 10 + (Math.random() * 20),
    color: ['#C5A059', '#FDFCF8', '#F8E8E8'][i % 3]
  })), []);
  ...
  return (
    <section className="py-40 bg-luxury-black text-cream relative overflow-hidden group/pact">
      ...
      {/* Floating Dream Icons (Parallax) */}
      <div className="absolute inset-0 pointer-events-none z-10 overflow-hidden">
        <motion.div style={{ y: useTransform(smoothY, [0, 1], [100, -300]), willChange: "transform" }} className="absolute top-[20%] left-[10%] opacity-10"><Plane size={120} className="text-gold" /></motion.div>
        <motion.div style={{ y: useTransform(smoothY, [0, 1], [200, -400]), willChange: "transform" }} className="absolute top-[60%] right-[15%] opacity-10"><Dumbbell size={80} className="text-white" /></motion.div>
        <motion.div style={{ y: useTransform(smoothY, [0, 1], [50, -500]), willChange: "transform" }} className="absolute bottom-[20%] left-[5%] opacity-10"><Dog size={100} className="text-gold" /></motion.div>
        <motion.div style={{ y: useTransform(smoothY, [0, 1], [150, -250]), willChange: "transform" }} className="absolute top-[40%] right-[5%] opacity-10"><Cat size={60} className="text-white" /></motion.div>
      </div>

      {/* Background Decorative Elements */}
      <div className="absolute inset-0 opacity-5 pointer-events-none select-none z-0">
        <div className="text-[30rem] font-serif italic text-white absolute -top-40 -left-20 rotate-12 leading-none">30</div>
      </div>
      ...
      <div className="absolute inset-0 pointer-events-none z-20">
        {butterflies.map((b, i) => (
          <motion.div
            key={`stomach-butterfly-${i}`}
            initial={{ opacity: 0, scale: 0, x: `${b.xStart}%`, y: "80%" }}
            whileInView={{ 
              opacity: [0, 0.6, 0],
              scale: [0, 1.2, 0.8],
              x: [`${b.xStart}%`, `${b.xEnd}%`],
            ...
            style={{ willChange: "transform, opacity" }}
          >
            <Wind size={20 + Math.random() * 20} style={{ color: b.color }} />
          </motion.div>
        ))}
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 relative z-10">
        ...
        >
          ...
          >
            <Heart className="text-gold" fill="currentColor" size={48} />
          </motion.div>
          <span className="text-gold font-sans text-[10px] uppercase tracking-[1.2em] mb-4 block">A Soul's Commitment</span>
          <h2 className="font-display italic text-6xl sm:text-8xl lg:text-9xl mb-4 text-gradient-gold">The Eternal Pact</h2>
          <p className="font-script text-3xl text-white/40 mt-6 italic">Signed in the gym, sealed in the soul.</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
          {dealPoints.map((point, i) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="p-12 border border-white/5 rounded-[60px] bg-white/[0.02] backdrop-blur-3xl flex flex-col items-center text-center group hover:bg-white/[0.05] hover:border-gold/30 transition-all duration-700 shadow-2xl relative"
              style={{ transformStyle: "preserve-3d", willChange: "transform, opacity" }}
            >
              {/* Garden Grow Feature for Saturday Vow */}
              {point.title === "The Saturday Vow" && (
                <>
                  <BloomingFlower className="absolute -top-4 -left-4" delay={0.1} />
                  <BloomingFlower className="absolute -bottom-4 -right-4" delay={0.3} />
                  <BloomingFlower className="absolute top-1/2 -left-6" delay={0.5} />
                </>
              )}

              <div className="w-24 h-24 rounded-full bg-gold/5 border border-gold/10 flex items-center justify-center mb-10 group-hover:scale-110 group-hover:rotate-6 transition-all duration-700 shadow-[0_0_50px_rgba(197,160,89,0.15)] relative">
                <div className="absolute inset-0 bg-gold/10 rounded-full animate-ping opacity-20" />
                {point.icon}
              </div>
              <h3 className="font-display text-3xl text-gold italic mb-6 tracking-wide underline-offset-8 group-hover:underline transition-all">{point.title}</h3>
              <p className="text-lg font-serif italic text-white/60 leading-relaxed group-hover:text-white/90 transition-colors px-4">
                {point.description}
              </p>
              
              <div className="mt-10 flex gap-4 opacity-0 group-hover:opacity-100 transition-all duration-700 delay-100">
                {point.title === "Our Sanctuary" && (
                  <>
                    <Dog size={16} className="text-gold/40" />
                    <Cat size={16} className="text-gold/40" />
                  </>
                )}
                {point.title === "The Saturday Vow" && <Flower2 size={16} className="text-gold/40" />}
              </div>
            </motion.div>
          ))}
        </div>

        {/* The "Eternal First Choice" Section */}
        <motion.div 
          initial={{ opacity: 0, y: 100 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 1.5 }}
          className="mt-32 p-16 sm:p-24 border border-gold/20 rounded-[80px] text-center bg-gradient-to-br from-gold/[0.05] via-transparent to-transparent backdrop-blur-xl relative overflow-hidden shadow-[0_0_100px_rgba(197,160,89,0.1)]"
          style={{ willChange: "transform, opacity" }}
        >
          ...
          <h4 className="font-sans font-bold text-[10px] uppercase tracking-[1.5em] text-gold mb-10">Verification of Intent</h4>
          <p className="font-display text-3xl md:text-5xl italic text-cream leading-tight max-w-4xl mx-auto mb-12 relative z-10 transition-all duration-700">
            "You were the first soul I ever had the courage to choose for myself. Not because of a dare, not because of the world, but because <span className="text-gold">it was always you.</span>"
          </p>
          
          <p className="font-serif text-xl text-white/50 italic max-w-2xl mx-auto leading-relaxed mb-16 px-4 transition-all duration-700">
            Our deal isn't just a funny coincidence or a backup plan. It's the highest form of respect I can give to the person who understands me like no one else ever has. One dog, one cat, a kitchen full of light, and a heart that was yours from the very first gym session.
          </p>
          
          <div className="flex flex-col sm:flex-row items-center justify-center gap-12 sm:gap-24 text-gold/40">
            <div className="flex flex-col items-center gap-4 group cursor-pointer relative">
              ...
              <span className="text-[8px] uppercase tracking-[0.5em] group-hover:text-gold transition-colors duration-500">The Chosen One</span>
              ...
              <motion.div 
                 ...
                 className="absolute top-1/2 left-full -translate-y-1/2 h-[1px] bg-gradient-to-r from-gold to-transparent hidden sm:block pointer-events-none"
              />
            </div>
            <p className="font-script text-4xl text-gold/20 select-none hidden sm:block relative z-10">&</p>
            <div className="flex flex-col items-center gap-4 group cursor-pointer relative">
              {/* Connection Sparkle */}
              <motion.div
                initial={{ scale: 0 }}
                whileInView={{ scale: [0, 1, 0], x: [-100, 0] }}
                transition={{ delay: 3, duration: 1.5 }}
                className="absolute top-1/2 left-0 -translate-x-full h-2 w-2 bg-gold rounded-full blur-sm hidden sm:block"
              />
              ...
              <span className="text-[8px] uppercase tracking-[0.5em] group-hover:text-gold transition-colors duration-500">The Constant</span>
            </div>
          </div>
          
          <motion.div 
            className="mt-20 text-[10px] uppercase tracking-[1em] opacity-30 italic font-bold"
  ...
  return (
    <section className="min-h-screen flex flex-col items-center justify-center p-6 relative overflow-hidden bg-luxury-black text-white">
      ...
      <div className="relative z-10 text-center max-w-3xl">
        ...
        <motion.h2 initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} className="font-display text-5xl sm:text-7xl md:text-8xl mb-12 leading-tight tracking-tight uppercase">A Journey to <br/> <span className="text-gold italic normal-case">Freedom</span></motion.h2>
